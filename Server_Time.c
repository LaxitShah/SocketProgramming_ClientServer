...
{
    time_t begin, end,session_start,session_end;
    time(&begin);
	...
	while(1)
	{
        ...
        scanf("%[^\n]%*c",reply);
        if (!strcmp(reply, exit_msg))
        {
		    close (fd);//it is used to terminate the connection
		    break;
    	}
    ...
    printf("Time Measure  %ld in seconds",time_elapsed);
    close(new_socket);
